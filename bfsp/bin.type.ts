declare namespace Bfsp {
  namespace Bin {
    interface CommandConfig<
      ...
      R extends
        | readonly CommandConfig.Input[]
        | readonly (readonly CommandConfig.Input[])[] = any
    > {
      readonly params?: P;
      readonly rests?: R;
    }
    namespace CommandConfig {
      ...
      type FromInputType<T> = T extends "number"
        ? number
        : T extends "boolean"
        ? boolean
        : string;
    ...
    type GetParamsInputType<T> = T extends CommandConfig<infer P, infer _>
      ? P
      : never;
    type ToParamType<T> = T extends CommandConfig.OptionalInput<
      infer Name,
      infer Type,
      infer Req
    >
      ...
      : {};
    type ToParamsType<T> = T extends readonly [infer R, ...infer Rests]
      ? ToParamType<R> & ToParamsType<Rests>
      : {};
