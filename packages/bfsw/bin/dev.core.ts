...
import type { RollupWatcher } from "@bfchain/pkgm-bfsp";
import { buildBfsp } from "@bfchain/pkgm-bfsp";
import { $BfspUserConfig, getBfspUserConfig } from "@bfchain/pkgm-bfsp";
import {
  watchBfspProjectConfig,
  writeBfspProjectConfig,
} from "@bfchain/pkgm-bfsp";
import { BuildService } from "@bfchain/pkgm-bfsp";
import { watchDeps } from "@bfchain/pkgm-bfsp";
...
export const workspaceItemDoDev = async (options: {
  ...
  const subStreams = watchBfspProjectConfig(
    projectConfig,
    buildService,
    subConfigs
  );
  ...
  /// 监听项目变动
  let preViteConfigBuildOptions:
    | BFChainUtil.FirstArgument<typeof ViteConfigFactory>
    | undefined;
  ...
  const abortable = Closeable<string, string>("bin:dev", async (reasons) => {
    ...
    (async () => {
      ...
      if (
        isDeepStrictEqual(viteConfigBuildOptions, preViteConfigBuildOptions)
      ) {
        return;
      }
      ...
      viteLogger.info(
        chalk.green(`vite ${userConfig.userConfig.name} bundle start`)
      );
