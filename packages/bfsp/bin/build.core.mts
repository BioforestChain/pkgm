...
import { $BfspUserConfig, $getBfspUserConfig } from "../src/configs/bfspUserConfig.mjs";
import { $PackageJson, generatePackageJson } from "../src/configs/packageJson.mjs";
import { $TsConfig, generateTsConfig, writeTsConfig } from "../src/configs/tsConfig.mjs";
import { generateViteConfig } from "../src/configs/viteConfig.mjs";
 ...
 * 修复profile类型路径
 * ```ts
 * import xxx from ".abc.mjs"
 * // 修改为
 * import xxx from "./abc#<profile>.mjs"
 * ```
...
const rePathProfileImports = async (tsConfigPaths: { [path: string]: string[] }, baseDir: string, file: string) => {
  ...
  await writeFile(
    file,
    contents
      // import xxx from ".xxx.mjs"
      .replace(/(im|ex)port (.*) from ".(.*).mjs"/g, (_, p1, p2, p3) => {
        const p = getRealPath(p3);
        return `${p1}port ${p2} from ".{p}.mjs"`;
      })
      // import "#xxx"
